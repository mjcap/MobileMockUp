...
public class BluetoothPlugin extends CordovaPlugin
{
	...
	public static double hexToTemp(String hexString) {
		String s1 = hexString.substring(0, 2);
		String s2 = hexString.substring(2, 4);
		int k = Integer.parseInt(s1, 16);
		int l = Integer.parseInt(s2, 16);
		int j = 1;
		int i = k;
		...
		float f = j;
		float f1 = (float)(i * 256 + l) / 10F;

		return Double.parseDouble((new StringBuilder()).append(Float.valueOf(f * f1)).toString());
	}    
	/**
		...
		public boolean handleMessage(Message msg)
		{
			switch(msg.what)
			{
				...
				case BluetoothWrapper.MSG_READ:
				        ...
				        byte[] byteArray = msg.getData().getByteArray(BluetoothWrapper.DATA_BYTES);
				        String data = "";
		                        for (int i=0; i<byteArray.length; ++i) {
			                   data += String.format("%02X", byteArray[i]);
		                        }

					Log.d(LOG_TAG, "MSG_READ data="+data);
					String dataToReturn = "";
                                        if (data.length() >= 28){
					  double tempC = hexToTemp(data.substring(24, 28));  // or maybe 26,30 or 32,26
				          double tempF = hexToTemp(data.substring(10, 14));

					  dataToReturn = new String("c:"+tempC+" f:"+tempF);
					}

					Log.d(LOG_TAG, "CordovaLog MSG_READ returning ="+dataToReturn);
					//Log.e(LOG_TAG, "MSG_READ BluetoothPlugin.this._encoding="+ BluetoothPlugin.this._encoding +" data="+data);
					Log.d(LOG_TAG, "CordovaLog MSG_READ BluetoothPlugin.this._encoding="+ BluetoothPlugin.this._encoding +" dataToReturn="+dataToReturn);
					if(_ioCallback != null)
					{
						PluginResult result = new PluginResult(PluginResult.Status.OK, dataToReturn);
						result.setKeepCallback(true);
